...
#define PORT "31634" // The port users will be connecting to
...
void loadfile (map<string, vector< vector<int> > > &dataBase) {
   ...
   string dataFileLocation = "./dataB.txt";
...
int main()
{
   ...
   {
      fprintf(stderr, "Server B: getaddrinfo: %s\n", gai_strerror(rv));
      return 1;
   }
                               ...
                               (struct sockaddr *)&their_addr, &addr_len)) > 0)
   {
      ...
      if(strcmp(buf,"send country names") == 0) {
         ...
         while(currentSent < sendLen) {
            ...
            {
               perror("Server B: send to main server failed ");
               exit(1);
            }
         ...
         printf("\nServer B has received a request for finding possible friends of User %s in %s\n", userIds, country);
         ...
         if(!found) { // User not found
            ...
            printf("Server B found the following possible friends for User %s in %s: %s\n", userIds, country, wouserString.c_str());
         }
         ...
         if(!found) {
            ...
            printf("Server B has sent the result(s) to Main Server\n");
         }
      }
